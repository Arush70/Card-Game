...
As a user,
  I want to enter the number of players `n`, 
    so that the correct number of players and decks is created.
...
As a user,
  I want to give the location of a pack file,
    so that the cards to be dealt are loaded.
...
As a user,
  I want an invalid pack rejected,
    so that the game starts only with valid input.
...
As a user,
  I want hands dealt and decks filled in round-robin order,
    so that the starting state matches the specification.
...
### US5 — Concurrent play
As a user,
  I want every player to run as its own thread,
    so that play is simultaneous, not sequential.
...
As a user,
  I want a player holding four equal cards to win and end the game,
    so that the game terminates correctly.
...
As a user,
  I want the winner shown on the terminal as `player i wins`,
    so that I see the result immediately.
...
As a user,
  I want `2n` output files in the exact specified format,
    so that the game is fully recorded.
...
### US17 Immidiate win on initial deal
As a user,
  I want a player who is dealt four cards of the same value at the start to win
  immediately before any rounds are played,
    so that an already-winning hand is recognised without unnecessary play.

- **Accept:** the initial hands are checked before the players begin normal play; if a
player holds four cards of the same value, that player declares a win at once; all
player output files are still written, each containing exactly four lines (initial hand,
win, exit, final hand).

### US13 — Dialog-based input
As a user,
  I want to enter the number of players and pack location through JOptionPane
  dialog boxes,
      so that I can configure the game graphically as well as from the terminal.
...
As a user,
  I want each player to keep its preferred denomination and discard a
  non-preferred card,
      so that play follows the rules.
...
### US10 — No stagnation
As a user,
  I want a player never to hold a non-preferred card indefinitely,
    so that the game cannot stall.
...
As a user,
  I want each draw-and-discard treated as one atomic action,
    so that every player ends with four cards.
...
As a user,
  I want a clear message when a pack cannot be found or read,
...
As a user,
  I want a confirmation that output files were written,
...
As a user,
  I will not expect the system to resolve two players winning in the same
  instant; per the brief this rare case is handled by re-running.

### US16 — Optimal strategy
As a user,
  I will not expect optimal play; the specified strategy is deliberately simple.
